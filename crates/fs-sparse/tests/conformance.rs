//! Conformance suite for fs-sparse (placeholder).
//!
...
fn conformance_placeholder_smoke() {
    // Case id: fs-sparse/smoke-000. Verdict schema: JSON-lines on stdout when the
    // real harness lands; for now the test passing IS the verdict.
    assert_eq!(fs_sparse::VERSION, env!("CARGO_PKG_VERSION"));
}
