//! Conformance suite for fs-simd (placeholder).
//!
...
fn conformance_placeholder_smoke() {
    // Case id: fs-simd/smoke-000. Verdict schema: JSON-lines on stdout when the
    // real harness lands; for now the test passing IS the verdict.
    assert_eq!(fs_simd::VERSION, env!("CARGO_PKG_VERSION"));
}
