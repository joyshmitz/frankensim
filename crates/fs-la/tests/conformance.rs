//! Conformance suite for fs-la (placeholder).
//!
...
fn conformance_placeholder_smoke() {
    // Case id: fs-la/smoke-000. Verdict schema: JSON-lines on stdout when the
    // real harness lands; for now the test passing IS the verdict.
    assert_eq!(fs_la::VERSION, env!("CARGO_PKG_VERSION"));
}
