//! Conformance suite for fs-alloc (placeholder).
//!
...
fn conformance_placeholder_smoke() {
    // Case id: fs-alloc/smoke-000. Verdict schema: JSON-lines on stdout when the
    // real harness lands; for now the test passing IS the verdict.
    assert_eq!(fs_alloc::VERSION, env!("CARGO_PKG_VERSION"));
}
