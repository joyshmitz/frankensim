//! Conformance suite for fs-ivl (placeholder).
//!
...
fn conformance_placeholder_smoke() {
    // Case id: fs-ivl/smoke-000. Verdict schema: JSON-lines on stdout when the
    // real harness lands; for now the test passing IS the verdict.
    assert_eq!(fs_ivl::VERSION, env!("CARGO_PKG_VERSION"));
}
