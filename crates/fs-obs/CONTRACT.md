...
- `xtask check-casual-print` seeds each package's actual library target (the
  ...
  invocation backed by the compiler-provided `std` extern-prelude entry;
  listing a file never exempts unrelated functions.
...
- The scanner inventories the repository-owned `crates/`, `tools/`, and
  ...
  components, normalized-key collisions, and backslash-bearing module literals
  ...
  entry, and package-candidate enumeration is capped before collection growth.
  ...
  single-line TOML basic-string escapes; unsupported escapes, multiline strings,
  and inline package/lib tables refuse rather than disappear. Source count/bytes,
...
- From each admitted library root, ordinary `mod foo;`, direct `#[path]`, and
  production `include!` edges with one bounded literal `.rs` path are followed
  transitively, including otherwise CLI-shaped targets. Missing or dual
  `foo.rs`/`foo/mod.rs` targets, package escapes, nonliteral/generated or non-Rust
  includes, cycles, aliases, conditional `cfg_attr(..., path=...)`, and
  unsupported inline-module path contexts refuse the audit rather than guessing.
  Test-only edges are excluded in either attribute order, and an iterative
  active/done traversal preserves cycle/alias diagnostics without ancestry
  cloning. A braced `macro_rules!` definition or item-macro invocation ends its
  own `cfg(test)` item at the balanced brace and cannot hide the following
  production function or module. Allowances require one production owner and
  ordered absolute invocations; protected imports/declarations/renames, legacy
  or selective `macro_use`, explicit `std` extern-crate rebinding, potentially
  external glob leaves, and direct or nested conditional `no_std`/`no_core`/
  `no_implicit_prelude` cannot lend them authority. Recursively grouped glob
  ...
  generated by other external or generated macros whose protected spelling is
  absent from inventoried workspace source.
