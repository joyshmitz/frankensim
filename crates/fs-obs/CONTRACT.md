...
- `xtask check-casual-print` recursively discovers every package below the
  owned `crates/`, `tools/`, and `xtask/` roots, seeds its actual library target
  (the default `src/lib.rs` or an explicit `[lib] path`, even when that path is
...
- From each admitted library root, ordinary `mod foo;`, direct `#[path]`, and
  ...
  or glob authority refuse instead of silently selecting an expansion. Missing or dual
