//! Conformance suite for fs-fft (placeholder).
//!
...
fn conformance_placeholder_smoke() {
    // Case id: fs-fft/smoke-000. Verdict schema: JSON-lines on stdout when the
    // real harness lands; for now the test passing IS the verdict.
    assert_eq!(fs_fft::VERSION, env!("CARGO_PKG_VERSION"));
}
