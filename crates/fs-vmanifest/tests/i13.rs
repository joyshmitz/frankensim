...
fn i13_terminal_v2_authority_is_acyclic_closed_and_capacity_complete() {
    ...
    for exact in [
        ...
        "InfrastructureCapacityReservationSetBytesV3 is the 34 exact ASCII bytes I13_INFRASTRUCTURE_CAPACITY_SET_V3 followed by one byte 0x00, raw32(predecessor_manifest_digest), U64LE(attempt_id), U64LE(epoch), U16LE(member_count=8), and eight FRAME_BYTES(one byte service_kind||raw32(service_identity)||raw32(owner_identity)||raw32(implementation_identity)||raw32(durable_medium_identity)||raw32(failure_domain_identity)||U64LE(reserved_bytes)||raw32(capacity_reservation_receipt_digest)||raw32(retention_policy_digest_or_zero)||raw32(destination_roster_digest_or_zero)) rows in service-kind order, exactly 2269 bytes",
        "exactly 546 bytes",
...
fn i13_v4_finite_capacity_wal_and_closed_evidence_authority_is_exact() {
    ...
    for exact in [
        ...
        "first durable Prepare for a new subject atomically increments reserved_subject_count and assigns physical_ordinal=old_reserved_subject_count",
        ...
        "exact signed/interval-bearing CapacityIndexAllocationOutcomeBytesV4 is appended as phase3 to every available preprovisioned decision journal before any newly allocated response log is trusted",
        "initial outcome -> reconciliation Prepare -> orphan member/proof/candidate root -> reconciliation local terminal -> final outcome",
    ] {
        assert!(
