//! Conformance suite for fs-rand (placeholder).
//!
...
fn conformance_placeholder_smoke() {
    // Case id: fs-rand/smoke-000. Verdict schema: JSON-lines on stdout when the
    // real harness lands; for now the test passing IS the verdict.
    assert_eq!(fs_rand::VERSION, env!("CARGO_PKG_VERSION"));
}
