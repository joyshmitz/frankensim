...
| `fs-ir` | FrankenScript typed AST with spans, isomorphic s-expression and JSON syntaxes, shape comparison, study recognition, lowering, structured IR errors, durable Machine-IR entity/topology lineage, dependency-neutral machine-graph admission, a separately admitted behavior overlay for typed state/IC/BC closure, motion/events/tolerances/dependence, and an assurance overlay binding admitted V&V cases, sensors/experiments, hazards/faults, signed accounting windows, and terminating fidelity escalation |
