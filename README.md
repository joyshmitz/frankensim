...
| L4 | Experimentally validated | 0 | The V&V corpus registry contains 19 Level-A thermal definitions/targets, a synthetic Level-B fixture, and a retained Level-C Martin-Moyce curve. Fifteen Level-A rows now have test-time executing-kernel bindings, but none persists a corpus receipt or machine fingerprint; the experimental row also lacks original raw data, metrology, replayable digitization lineage, resolved redistribution authority, and a defensible scalar envelope. Every row is capped `Estimated`, so no L4 claim exists |
